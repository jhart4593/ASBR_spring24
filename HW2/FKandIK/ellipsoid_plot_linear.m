function [eigenvalues, eigenvectors] = ellipsoid_plot_linear(Jacobian)
%+ Given the geometric Jacobian (either Jb in the end-effector frame or 
% Js in the fixed frame) plot the linear manipulability ellipsoid
...
if rank(Jv) < size(Jacobian,1)/2
     warning('Linear Jacobian is not full rank, singularity is present');
end
