function [endEffectorPos, mags, VbIter, JointAngleIter] = J_transpose_kinematics(BScrews,M,initialAngle,desiredAngle)
...
% compute the inverse kinematics numerically using the J transpose method
%+ Returns:
...
while magW > ew || magV > ev
    ...
    Vb = S*ang; % Vb
    JJte = JacobianBody*JacobianBody'*Vb; 
    alpha = dot(Vb,JJte)/dot(JJte,JJte);
    ...
    theta = theta+alpha*JacobianBody'*Vb;
    i = i+1;
